...
Imports the root function from the scipy.optimize module. scipy is a library in Python used for scientific computing, and root is a function within the optimize module specifically designed for finding roots of equations numerically.
Defining the Equation:

Defines a Python function named equation which takes a single argument x. Inside the function, it returns the result of the equation x**3 - 2*x - 5.
Getting Initial Guess from the User:

Enter an initial guess for the root of the equation. The input is converted to a floating-point number (float) since the input function returns a string by default.
Finding the Root:
...
solGuess = root(eqn, guess)

print(f"Root: {solGuess.x}")