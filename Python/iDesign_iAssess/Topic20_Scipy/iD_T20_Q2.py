...
xInp = input("Enter the values of x separated by spaces: ")
yInp = input("Enter the values of y separated by spaces: ")

xVal = list(map(float, xInp.split()))
yVal = list(map(float, yInp.split()))

xArr = np.array(xVal)
yArr = np.array(yVal)

slope, intercept, r_value, p_value, std_err = linregress(xArr, yArr)
rSqr = r_value ** 2

print(f"\nSlope: {slope}")
print(f"Intercept: {intercept}")
print(f"R-squared: {rSqr}")
